...
import { Zap, AlertTriangle, Users, TrendingUp, Clock, Layers, ShieldAlert, CheckCircle2 } from 'lucide-react';
import { getAppState, saveAppState, getDynamicTrades } from '../../lib/dbState';
import { syncTaskToBackend } from '../../lib/backendSync';

export const ResourceAllocationCenter = () => {
  ...
  const handleDeployLaborToFloor = (floorNumber, trade) => {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        ...
        <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl">
          ...
          <div className="text-3xl font-black font-mono text-amber-400 mt-2">
            {(state.flatTasks || []).length > 0 ? Math.round(((state.flatTasks || []).filter(t => t.status === 'APPROVED').length / state.flatTasks.length) * 100) : 0}%
          </div>
            ...
            {activeBlockers.map((t) => {
              ...
              return (
                <div key={t.id} className="p-3.5 bg-slate-950 border border-rose-900/60 rounded-xl text-xs flex flex-wrap items-center justify-between gap-3">
                  ...
                  <button
                    onClick={() => {
                      const updatedTask = { ...t, status: 'IN_PROGRESS', blockerReason: undefined, updatedAt: new Date().toISOString() };
                      saveAppState({
                        ...state,
                        flatTasks: (state.flatTasks || []).map(task => task.id === t.id ? updatedTask : task)
                      });
