...
export const ElevatorGrid = ({
  ...
  const floors = Array.from(new Set(wingFlats.map(f => f.floorNumber))).sort((a, b) => b - a);

  const flatNumbers = wingFlats.map(f => parseInt(f.flatNumber.slice(-1)));
  const flatIndices = Array.from(new Set(flatNumbers.filter(n => !isNaN(n)))).sort((a, b) => a - b);
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800 pb-3">
        <div>
          ...
          <p className="text-xs text-slate-400">
            Building visual matrix across all {wingFlats.length} flats (Filter trade completion velocity)
          </p>
        </div>
