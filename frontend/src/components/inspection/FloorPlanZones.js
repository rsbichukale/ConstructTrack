...
export const FloorPlanZones = ({
  ...
  const taskCatalogToUse = state.taskCatalog || [];

  const availableZones = (state.roomZones || []).filter(zone => {
    if (flat.flatType === '2BHK') {
      return zone.id <= 9;
    }
    return true;
  });
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-bold text-white flex items-center space-x-2">
            ...
            <span>Flat {flat.flatNumber} ({flat.flatType}) Floor Plan Zones</span>
          </h2>
      ...
      <div className="bg-slate-900 border border-slate-800 p-4 sm:p-6 rounded-2xl space-y-4">
        <div className="text-xs font-semibold text-sky-400 uppercase tracking-wider flex items-center justify-between">
          <span>{flat.flatType} Architectural Layout Map (Flat {flat.flatNumber})</span>
          <span className="text-slate-500 font-mono text-[11px]">{availableZones.length} Room Zones</span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {availableZones.map((zone) => {
            ...
            const catalogItems = taskCatalogToUse.filter(c => c.roomZoneId === zone.id);
            ...
            const totalTasks = catalogItems.length;
